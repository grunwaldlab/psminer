...
#'
#' @param path The path to one or more pathogensurveillance output folders.
#' @return character vector of length 1
#' @family path finders
#' @export
sample_meta_path <- function(path) {
  find_static_file_path(path, 'sample_data.csv', 'sample metadata file')
}
...
ref_meta_path <- function(path) {
  find_static_file_path(path, 'reference_data.csv', 'reference metadata file')
}
...
busco_tree_path <- function(path) {
  find_static_file_path(path, 'busco_tree.nwk', 'busco tree file')
}
...
busco_ref_path <- function(path) {
  find_static_file_path(path, 'busco_tree_references.csv', 'busco anaylsis reference file')
}
...
core_ref_path <- function(path) {
  find_static_file_path(path, 'core_gene_tree_references.csv', 'core gene analysis reference file')
}
...
#' @family path finders
#' @export
report_group_path <- function(path) {
  find_static_file_path(path, 'group_id.txt', 'report group name file')
}
...
variant_ref_path <- function(path) {
  find_static_file_path(path, 'mapping_references.csv', 'variant analysis refernece file')
}
...
status_message_path <- function(path) {
  find_static_file_path(path, 'messages.csv', 'status message data file')
}
...
pocp_matrix_path <- function(path) {
  find_static_file_path(path, 'pocp.csv', 'pocp matrix file')
}
...
#' @family path finders
#' @export
core_tree_paths <- function(path) {
  find_static_dir_paths(path, 'core_gene_trees', 'core gene tree', file_required = FALSE)
}
...
#' @family path finders
#' @export
considered_ref_meta_paths <- function(path) {
  find_static_dir_paths(path, 'ncbi_reference_data', 'considered NCBI reference metadata')
}
...
#' @family path finders
#' @export
selected_ref_meta_paths <- function(path) {
  find_static_dir_paths(path, 'selected_references', 'selected NCBI reference metadata')
}
...
#' @family path finders
#' @export
sendsketch_paths <- function(path) {
  find_static_dir_paths(path, 'sendsketch', 'sendsketch result')
}
...
#' @family path finders
#' @export
variant_align_paths <- function(path) {
  find_static_dir_paths(path, 'snp_alignments', 'SNP alignment')
}
...
#' @family path finders
#' @export
variant_tree_paths <- function(path) {
  find_static_dir_paths(path, 'snp_trees', 'SNP tree')
}
