...
#'                 (when `sample_id` is present) or a report group (when `sample_id` is NA).
#' @param interactive Logical; indicates whether to produce interactive tables (TRUE) or static tables (FALSE).
...
#'
#'
#' @return A list containing three tables: `simpleTable`, `detailedSamplesTable`, and `detailedGroupsTable`.
#'         `simpleTable` is a high-level summary of pipeline steps and associated issues.
#'         `detailedSamplesTable` and `detailedGroupsTable` provide in-depth insights into issues at the sample and
...
#'
#' @importFrom dplyr filter group_by summarise left_join select mutate case_when
