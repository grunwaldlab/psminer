...
plot_phylogeny <- function(trees, sample_meta, ref_meta, color_by = NULL, collapse_by_tax = NULL, interactive = knitr::is_html_output(), ...) {
  ...
  if (! all(is_color(tip_colors))) {
    ...
    factor_key <- stats::setNames(viridis::viridis(length(factors)), factors)
  ...
  tip_labels <- gsub(tip_labels, pattern = ':', replacement = '-', fixed = TRUE)
  tip_labels <- gsub(tip_labels, pattern = ',', replacement = '.', fixed = TRUE)
  tip_labels <- gsub(tip_labels, pattern = '_', replacement = ' ', fixed = TRUE)
  ...
  phycanv$x$tree <- gsub(phycanv$x$tree, pattern = '_', replacement = ' ', fixed = TRUE) # Remove underscores from tip labels (phylocanvas converts spaces to underscores)

  for (x in combined_tree$tip.label) { # NOTE: using the names instead of IDs like this could cause bugs if the name is not unique
    print(x)
    print( unname(tip_colors[x]))
    phycanv <- style_node(phycanv, x, labelcolor = unname(tip_colors[x]), labeltextsize = 30)
  }
}
